...
class TestRunner {
    ...
    public static void main(String[] args) {
        ...
        test("track() passes the event name correctly", () -> {
            ...
            assertTrue(entry.startsWith("page_view:"), "recorded entry should start with event name — got: \"" + entry + "\"");
        });
        test("track() passes the data map correctly", () -> {
            ...
            String entry = sdk.getRecorded().get(0);
            assertTrue(entry.contains("book"),  "recorded entry should contain 'book' — got: \"" + entry + "\"");
            assertTrue(entry.contains("price"), "recorded entry should contain 'price' — got: \"" + entry + "\"");
        });
        ...
        test("adapter works when used as a Tracker reference", () -> {
            ...
            assertTrue(!sdk.getRecorded().isEmpty(), "event should reach the SDK through the Tracker interface — got: " + sdk.getRecorded());
        });
