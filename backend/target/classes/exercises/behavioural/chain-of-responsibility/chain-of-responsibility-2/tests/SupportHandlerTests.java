class TestRunner {
    ...
    public static void main(String[] args) {
        test("L1 resolves severity-1 ticket", () ->
            assertTrue(buildChain().handle(new Ticket("Password reset", 1)).contains("L1"), "L1 should handle severity 1"));
        test("L2 resolves severity-2 ticket", () ->
            assertTrue(buildChain().handle(new Ticket("Database error", 2)).contains("L2"), "L2 should handle severity 2"));
        test("L3 resolves severity-3 ticket", () ->
            assertTrue(buildChain().handle(new Ticket("System outage", 3)).contains("L3"), "L3 should handle severity 3"));
        test("result contains the ticket issue", () ->
            assertTrue(buildChain().handle(new Ticket("System outage", 3)).contains("System outage"), "result should mention ticket issue"));
        test("severity-1 ticket is not escalated to L2 or L3", () -> {
            String r = buildChain().handle(new Ticket("FAQ question", 1));
            assertTrue(!r.contains("L2") && !r.contains("L3"), "severity-1 should stay at L1");
        });
        test("severity-2 ticket bypasses L1 and is handled by L2", () -> {
            String r = buildChain().handle(new Ticket("Network issue", 2));
            assertTrue(!r.contains("L1"), "severity-2 should not be handled by L1");
            assertTrue(r.contains("L2"),  "severity-2 should be handled by L2");
        });
        test("severity-3 ticket bypasses L1 and L2", () -> {
            String r = buildChain().handle(new Ticket("Outage", 3));
            assertTrue(!r.contains("L1") && !r.contains("L2"), "severity-3 should not be handled by L1 or L2");
        });
