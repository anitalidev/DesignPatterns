...
class TestRunner {
    ...
    public static void main(String[] args) {
        ...
        test("both iterators implement TreeIterator and are interchangeable", () -> {
            TreeNode root = tree();
            TreeIterator level = new LevelOrderIterator(root);
            TreeIterator pre   = new PreOrderIterator(root);
            // same collect() call works for both — caller code does not change
        ...
        test("two independent iterators on the same tree do not interfere", () -> {
            TreeNode root = tree();
            LevelOrderIterator it1 = new LevelOrderIterator(root);
            LevelOrderIterator it2 = new LevelOrderIterator(root);
            assertEquals(1, it1.next(), "it1 first");
            assertEquals(1, it2.next(), "it2 first — independent of it1");
